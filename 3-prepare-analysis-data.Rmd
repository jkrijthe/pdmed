...
## Imputation
```{r}
vars_impute <- c("MDS_UPDRS_I","MDS_UPDRS_II","MDS_UPDRS_III_other","MSEADLG",df_repeated_a %>% names %>% str_subset("PC.*_"))
...
df_last_tv <- df_varying %>% 
    ...
    select(id,Year,starts_with("MDS"),MoCA,MSEADLG,QUIP,QUIP8,QUIPany,NP4WDYSK,NP4OFF,LEDD) %>% 
    as.data.frame %>% 
    reshape(idvar = "id", timevar="Year", direction="wide", sep="_year")  %>%
  select(id,starts_with("MDS_"),starts_with("MoCA"),starts_with("MSEADLG"),starts_with("QUIP"),starts_with("NP4WDYSK"),starts_with("NP4OFF"),starts_with("LEDD"))
