...
if (file.exists("data/results-summary.RData")) {
  ...
  full_results_onldopa_levoonly <- bind_rows(
    full_results_onldopa_levoonly_y4 %>% filter(str_detect(outcome,"_year2")),
...
cairo_ps("1.eps", width=6, height = 0.75*6)
print(ggout)
dev.off()
print(ggout)
```
...
cairo_ps("2.eps", width=8, height = 0.3*8)
print(ggout)
dev.off()
print(ggout)
```
...
plot_bootstrap_compact <- function(df_res, df_boot) {
  ...
  df_res %>% 
    ...
    mutate(Model=fct_rev(fct_inorder(Model))) %>% 
    ggplot(aes(x=fct_rev(Year),y=estimate,color=Model,shape=Model)) +
...
cairo_ps("3.eps", width=6, height = 0.4*6)
print(ggout)
dev.off()
print(ggout)
```
...
cairo_ps("4.eps", width=8, height = 0.4*8)
print(ggout)
dev.off()
print(ggout)
```
