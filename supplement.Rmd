...
```{tikz dag,fig.ext="png",fig.width=3, fig.cap="Figure 1S: Directed Acyclic Graph of an idealized version of our causal model. L represents the set of covariates and outcomes (MDS-UPDRS scores, MoCA, MSE-ADL, etc.) and T represents the treatment indicator. In addition, we assume there are background variables (age, sex, education, time since diagnosis) that may influence both treatment decisions and the covariates in L, and the full history of treatment decisions may influence the current disease state (not shown)."}
...
(8) edge                   node {} (4)
(9) edge                   node {} (5);
         % (6) edge [bend right=120pt] node {} (5);
  \end{tikzpicture}
```
...
df_varying %>% 
  ...
  left_join(treat_length,by="id") %>% 
  filter(time==4) %>% 
  filter(is.na(MDS_UPDRS_III_off)) %>% 
  filter(!is.na(years_treatment)) %>% 
  select(years_treatment, MDS_UPDRS_III_on, MDS_UPDRS_III_other) %>% View()
...
bind_rows("2"=dfuse2,"1.5"=dfuse15,"1"=dfuse1,"0.5"=dfuse05,.id = "Treatment") %>% 
  ...
  kable(digits = 2, caption="Table 3S: Medication used during the first visit where the patients indicated that they used PD medication, according to the concomitant medication log. Numbers indicate what fraction of the patients used that medication in each group (0.5 year, 1 year, 1.5 year and 2 years of using PD medication in the first two years of the disease).")
```

## MDS-UPDRS, MoCA, MSEADL results
```{r}
plot_bootstrap <- function(df_res, df_boot) {
  df_res %>% 
    ...
    geom_point() +
    geom_segment(aes(x=estimate-2*sd,xend=estimate+2*sd,yend=Model),alpha=0.6) +
    theme_bw() +
...
```{r fig.asp=0.5, fig.cap="Figure 2S. Effect of a year of PD medication treatment during the first two years of follow-up (estimated using all patients). Effect on the score of different outcomes measured at year 2, 3 and 4. "}
...
plot_bootstrap(
  full_results_PDMEDYN %>% process_results,
  bs_pdmedyn_all %>% process_results
)
```

```{r fig.asp=0.5, fig.cap="Figure 3S: Effect of a year of PD medication treatment during the first two years of follow-up (estimated using only those patients who started during the first two years). Effect on the score of different outcomes measured at year 2, 3 and 4."}
plot_bootstrap(
...
```{r fig.asp=0.4, fig.width=9, fig.cap="Figure 4S: Effect of a year of PD medication treatment during the first two years of follow-up (estimated using only those patients who started during the first two years). Effect on the score of different outcomes measured at year 2, 3 and 4."}
plot_bootstrap(
...
```{r fig.asp=0.5, fig.cap="Figure 5S: Effect of a year of Levodopa treatment during the first two years of follow-up (estimated using only those patients who used levodopa exclusively during the first two years). Effect on the score of different outcomes measured at year 2, 3 and 4."}
plot_bootstrap(
...
```{r fig.asp=0.5, fig.cap="Figure 6S: Effect of a year of Levodopa treatment during the first two years of follow-up (estimated using all patients). Effect on the score of different outcomes measured at year 2, 3 and 4."}
plot_bootstrap(
  full_results_onldopa %>% process_results,
  bs_onldopa_all %>% process_results
)
```

```{r fig.asp=0.5, fig.cap="Figure 7S: Effect of a year of Levodopa treatment during the first two years of follow-up (estimated using only those patients who started during the first two years). Effect on the score of different outcomes measured at year 2, 3 and 4."}
plot_bootstrap(
  full_results_onldopa_started %>% process_results,
  bs_onldopa_started %>% process_results
)
```

## Other outcomes

```{r fig.asp=0.4, fig.width=9, fig.cap="Figure 8S: Effect of a year of Levodopa treatment during the first two years of follow-up (estimated using only those patients who used levodopa exclusively during the first two years). Effect on the score of different outcomes measured at year 2, 3 and 4."}
plot_bootstrap(
...
```{r fig.asp=0.4, fig.width=9, fig.cap="Figure 9S: Effect of a year of Levodopa treatment during the first two years of follow-up (estimated using only those patients who started during the first two years). Effect on the score of different outcomes measured at year 2, 3 and 4."}
plot_bootstrap(
  full_results_onldopa_started %>% process_results_aux,
  bs_onldopa_started %>% process_results_aux
)
```
...
```{r fig.asp=0.4, fig.cap="Figure 11S: Additive effect of treatment being active at different time-points with PD medication treatment during the first two years for the IPTW model for only those patients who started treatment (hence all have active treatment at time 2). Effect on the score of different outcomes measured at year 2, 3 and 4."}
plot_bootstrap(
  full_results_PDMEDYN_started %>% process_results,
  bs_pdmedyn_started %>% process_results
)
...
```{r fig.asp=0.4, fig.cap="Figure 12S: Additive effect of treatment being active at different time-points with PD medication treatment during the first two years for the g-formula model  (estimated using all patients). Effect on the score of different outcomes measured at year 2, 3 and 4."}
process_results <- function(.x) {
  .x %>% 
    filter(term %in% c("treatment","treatment_lag1","treatment_lag2","treatment_lag3")) %>% 
...
# Effect modification
To estimate the potential modification of the effect of different baseline characteristics of the patient, using the inverse probability weights for each outcome/year we estimate a marginal structural model containing main effects and interaction terms with treatment length for age, sex, disease duration and three MDS-UPDRS subscores at baseline in a single model.

```{r fig.cap="Figure 13S: Interaction effects in the IPTW Marginal Structural Model (main effects not shown), for the length of PD medication treatment in the first two years of follow-up using all patients in the analysis (estimated using all patients)."}
process_results <- function(.x) {
  .x %>% 
